...
pointToIndex width height (Point x y) =
  let wshift = fromIntegral (div width 2) :: Coord
      ...
      px = fromReal ((x' * wshift) + wshift) :: Int 
      py = fromReal ((y' * hshift) + hshift) :: Int
  in 
    px + py * width

plotWithPalette :: Palette -> Coord -> Color
...
colorToPixel (Color r g b a) =
  (Pixel
    ...
    convert g
    convert b)
  where convert channel = fromReal $ channel * fromIntegral intChannelMax
